...
public struct StderrOutputStream: OutputStream {
    ...
    fileprivate static var fpos : fpos_t = 0
    fileprivate static var fd = dup(fileno(stderr))
    fileprivate static var isRedirected_ = false
    public static var isRedirected : Bool {return isRedirected_}

    // When false, the output file is rewritten and not appended
    fileprivate static var appendToFile = true
    ...
    public static func redirectOutputToPath(_ path: String?) {
        if let path = path {
            if !isRedirected {
                // Set up if this is new redirection
                fflush(stderr)
                let pos = UnsafeMutablePointer<fpos_t>(allocatingCapacity: 1)
                fgetpos(stderr, pos)
                fpos = pos.pointee
                free(pos)
...
public class Logger {
    fileprivate static let stream = StderrOutputStream()
