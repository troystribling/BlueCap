...
public struct TiSensorTag {
    ...
    public struct MovementService: ServiceConfigurable {
        ...
        public struct Data: RawArrayDeserializable, CharacteristicConfigurable, StringDeserializable {
            ...
            public let gyroscopePoint: Point3D
            public let accelerometerPoint: Point3D
            public let magnetometerPoint: Point3D

            let gyroscopeRawPoint: RawPoint3D
            let accelerometerRawPoint: RawPoint3D
            let magnetometerRawPoint: RawPoint3D

            public init?(gyroscopePoint: Point3D, accelerometerPoint: Point3D, magnetometerPoint: Point3D) {
                self.gyroscopePoint = gyroscopePoint
                gyroscopeRawPoint = Data.rawPoint3DFromPoint3D(self.gyroscopePoint)

                self.accelerometerPoint = accelerometerPoint
                accelerometerRawPoint = Data.rawPoint3DFromPoint3D(self.accelerometerPoint)

                self.magnetometerPoint = magnetometerPoint
                magnetometerRawPoint = Data.rawPoint3DFromPoint3D(self.magnetometerPoint)
            }

            fileprivate static func rawPointFromRawValues(_ rawValues: [Int8], _ position: Int) -> Int16 {
                return Int16(rawValues[position]) | Int16(((UInt16(rawValues[position + 1]) << 8) & 0xff00))
            }

            fileprivate static func rawValueFromRawPoint(_ rawPoint: Int16) -> [Int8] {
                //                let lsb = Int8((rawPoint >> 8) & 0x00ff)
                //                let msb = Int8(rawPoint & 0x00ff)
                return [0, 0]
            }

            fileprivate static func pointFromRawPoint(_ rawPoint: Int16) -> Double {
                return 255.0 * Double(rawPoint) / 32768.0
            }

            fileprivate static func rawPointFromPoint(_ point: Double) -> Int16 {
                return Int16(32768.0 * point / 255.0)
            }

            fileprivate static func rawPoint3DFromRawValues(_ rawValues: [Int8], _ position: Int) -> RawPoint3D {
                return RawPoint3D(x: rawPointFromRawValues(rawValues, position),
                                  y: rawPointFromRawValues(rawValues, position + 2),
                                  z: rawPointFromRawValues(rawValues, position + 4))
            }

            fileprivate static func point3DFromRawPoint3D(_ rawPoint: RawPoint3D) -> Point3D {
                return Point3D(x: pointFromRawPoint(rawPoint.x), y: pointFromRawPoint(rawPoint.y), z: pointFromRawPoint(rawPoint.z))
            }

            fileprivate static func rawPoint3DFromPoint3D(_ point: Point3D) -> RawPoint3D {
                return RawPoint3D(x: rawPointFromPoint(point.x), y: rawPointFromPoint(point.y), z: rawPointFromPoint(point.z))
            }

            fileprivate static func rawValuesFromRawPoint3D(_ rawPoint: RawPoint3D) -> [Int8] {
                return rawValueFromRawPoint(rawPoint.x) + rawValueFromRawPoint(rawPoint.y) + rawValueFromRawPoint(rawPoint.z)
            }

            // CharacteristicConfigurable
            public static let uuid = "F000AA81-0451-4000-B000-000000000000"
            public static let name = "Movement Data"
            public static let properties: CBCharacteristicProperties = [.read, .notify]
            public static let permissions: CBAttributePermissions = [.readable, .writeable]
            public static let initialValue: Foundation.Data? = SerDe.serialize(Data(gyroscopePoint: Point3D(x: 1.0, y: 1.0, z: 1.0),
                                                                                    accelerometerPoint: Point3D(x: 1.0, y: 1.0, z: 1.0),
                                                                                    magnetometerPoint: Point3D(x: 1.0, y: 1.0, z: 1.0))!)
            ...
            public init?(rawValue: [Int8]) {
                ...
                gyroscopeRawPoint = Data.rawPoint3DFromRawValues(rawValue, 0)
                gyroscopePoint = Data.point3DFromRawPoint3D(gyroscopeRawPoint)

                accelerometerRawPoint = Data.rawPoint3DFromRawValues(rawValue, 6)
                accelerometerPoint = Data.point3DFromRawPoint3D(accelerometerRawPoint)

                magnetometerRawPoint = Data.rawPoint3DFromRawValues(rawValue, 12)
                magnetometerPoint = Data.point3DFromRawPoint3D(gyroscopeRawPoint)
            }

            public var rawValue: [Int8] {
                return Data.rawValuesFromRawPoint3D(gyroscopeRawPoint) +
                    Data.rawValuesFromRawPoint3D(accelerometerRawPoint) +
                    Data.rawValuesFromRawPoint3D(magnetometerRawPoint)
            }
            ...
            public var stringValue: [String: String] {
                return ["gyroscope-x" : NSString(format: "%.2f", gyroscopePoint.x) as String,
                        "gyroscope-y" : NSString(format: "%.2f", gyroscopePoint.y) as String,
                        "gyroscope-z" : NSString(format: "%.2f", gyroscopePoint.z) as String,
                        "gyroscope-xRaw" : "\(gyroscopeRawPoint.x)",
                        "gyroscope-yRaw" : "\(gyroscopeRawPoint.y)",
                        "gyroscope-zRaw" : "\(gyroscopeRawPoint.z)",

                        "accelerometer-x" : NSString(format: "%.2f", accelerometerPoint.x) as String,
                        "accelerometer-y" : NSString(format: "%.2f", accelerometerPoint.y) as String,
                        "accelerometer-z" : NSString(format: "%.2f", accelerometerPoint.z) as String,
                        "accelerometer-xRaw" : "\(accelerometerRawPoint.x)",
                        "accelerometer-yRaw" : "\(accelerometerRawPoint.y)",
                        "accelerometer-zRaw" : "\(accelerometerRawPoint.z)",

                        "magnetometer-x" : NSString(format: "%.2f", magnetometerPoint.x) as String,
                        "magnetometer-y" : NSString(format: "%.2f", magnetometerPoint.y) as String,
                        "magnetometer-z" : NSString(format: "%.2f", magnetometerPoint.z) as String,
                        "magnetometer-xRaw" : "\(magnetometerRawPoint.x)",
                        "magnetometer-yRaw" : "\(magnetometerRawPoint.y)",
                        "magnetometer-zRaw" : "\(magnetometerRawPoint.z)"]
            }

            public init?(stringValue: [String: String]) {
                guard let gyroscopeXRaw = int16ValueFromStringValue("gyroscope-xRaw", values: stringValue),
                    let gyroscopeYRaw = int16ValueFromStringValue("gyroscope-yRaw", values: stringValue),
                    let gyroscopeZRaw = int16ValueFromStringValue("gyroscope-zRaw", values: stringValue) else {
                        return nil
                }
                guard let accelerometerXRaw = int16ValueFromStringValue("accelerometer-xRaw", values: stringValue),
                    let accelerometerYRaw = int16ValueFromStringValue("accelerometer-yRaw", values: stringValue),
                    let accelerometerZRaw = int16ValueFromStringValue("accelerometer-zRaw", values: stringValue) else {
                        return nil
                }
                guard let magnetometerXRaw = int16ValueFromStringValue("magnetometer-xRaw", values: stringValue),
                    let magnetometerYRaw = int16ValueFromStringValue("magnetometer-yRaw", values: stringValue),
                    let magnetometerZRaw = int16ValueFromStringValue("magnetometer-zRaw", values: stringValue) else {
                        return nil
                }
                gyroscopeRawPoint = RawPoint3D(x: gyroscopeXRaw, y: gyroscopeYRaw, z: gyroscopeZRaw)
                gyroscopePoint = Data.point3DFromRawPoint3D(gyroscopeRawPoint)

                accelerometerRawPoint = RawPoint3D(x: accelerometerXRaw, y: accelerometerYRaw, z: accelerometerZRaw)
                accelerometerPoint = Data.point3DFromRawPoint3D(accelerometerRawPoint)

                magnetometerRawPoint = RawPoint3D(x: magnetometerXRaw, y: magnetometerYRaw, z: magnetometerZRaw)
                magnetometerPoint = Data.point3DFromRawPoint3D(magnetometerRawPoint)
            }

        }

        // Movement Config
        public enum Config: UInt16, RawDeserializable, StringDeserializable, CharacteristicConfigurable {
            ...
            public static let name = "Movement Config"
            public static let properties: CBCharacteristicProperties = [.read, .write]
            public static let permissions: CBAttributePermissions = [.readable, .writeable]
            public static let initialValue: Foundation.Data? = SerDe.serialize(Config.no.rawValue)
            ...
            public init?(stringValue: [String: String]) {
                guard let value = stringValue[Config.name] else {
                    return nil
                }
                switch value {
                case "Yes":
            ...
            public var stringValue: [String: String] {
                switch self {
                case .no:
                    return [Config.name : "No"]
                case .yes:
                    return [Config.name : "Yes"]
                }
            }
        ...
        // Movement Period
        public struct Period: RawDeserializable, CharacteristicConfigurable, StringDeserializable {
            ...
            // CharacteristicConfigurable
            public static let uuid = "F000AA83-0451-4000-B000-000000000000"
            public static let name = "Movement Period"
            public static let permissions: CBAttributePermissions = [.readable, .writeable]
            public static let properties: CBCharacteristicProperties = [.read, .write]
            public static let initialValue: Foundation.Data? = SerDe.serialize(UInt8(100))
            ...
            public init?(rawValue: UInt8) {
                ...
                self.period = Period.valueFromRaw(self.periodRaw)
            }
...
public class TISensorTagProfiles {
    ...
    public static func create(profileManager: ProfileManager) {
        ...
        // Movement Service
        let movementService = ConfiguredServiceProfile<TiSensorTag.MovementService>()
        let movementDataCharacteristic = RawArrayCharacteristicProfile<TiSensorTag.MovementService.Data>()
        let movementConfigCharacteristic = RawCharacteristicProfile<TiSensorTag.MovementService.Config>()
        let movementPeriodCharacteristic = RawCharacteristicProfile<TiSensorTag.MovementService.Period>()

        movementService.addCharacteristic(movementDataCharacteristic)
        movementService.addCharacteristic(movementConfigCharacteristic)
        movementService.addCharacteristic(movementPeriodCharacteristic)
        profileManager.addService(movementService)
    }
}
