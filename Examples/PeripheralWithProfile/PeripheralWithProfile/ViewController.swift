...
class ViewController: UITableViewController {
    ...
    var accelerometerService: BCMutableService {
        return BCMutableService(profile: BCConfiguredServiceProfile<TISensorTag.AccelerometerService>(), peripheralManager: self.manager)
    }
    ...
    func startAdvertising() {
        ...
        let powerOffFuture = self.manager.powerOff().flatmap { _ -> Future<Void> in
            if self.accelerometer.accelerometerActive {
                self.accelerometer.stopAccelerometerUpdates()
                self.enabledSwitch.on = false
            }
